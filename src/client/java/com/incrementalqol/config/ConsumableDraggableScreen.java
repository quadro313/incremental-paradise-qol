...
public class ConsumableDraggableScreen extends Screen {
    public static final Logger LOGGER = LoggerFactory.getLogger(MOD_ID);
    private final Screen parent;

    private final Config config = Config.HANDLER.instance();

    public ConsumableDraggableScreen(Screen parent) {
        super(Text.literal("Incremental Paradise QOL Options"));
        this.parent = parent;
    }

    public ConsumableHudWidget hud1;
    ...
    @Override
    protected void init() {
        hud1 = new ConsumableHudWidget(config.getConsumableHudPosX(), config.getConsumableHudPosY(), 100, 50);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        float scale = (float) config.getConsumableHudScale();
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double deltaX, double deltaY) {
        if (isDragging) {
            ...
            float scaleFactor = (float) config.getConsumableHudScale(); // Make sure this matches your rendering scale
            ...
            config.setConsumableHudPosX(newX);
            config.setConsumableHudPosY(newY);
